# General Rules

This file activates core rules that apply to all projects.

## Core Guidelines

These are fundamental rules that should be applied across all projects:

- Follow the project's established code style and patterns
- Maintain consistent documentation practices
- Implement proper error handling and logging
- Write tests for all new functionality
- Keep dependencies up to date and secure
- Follow security best practices in all implementations
- Ensure code is maintainable and readable
- Document architecture decisions clearly
- Follow version control best practices
- Monitor and optimize performance
- Ensure accessibility compliance
- Support internationalization from the start
